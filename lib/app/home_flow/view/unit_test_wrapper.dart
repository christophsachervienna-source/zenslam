...
class UnitTestWrapper extends StatefulWidget {
  final Widget child;
  const UnitTestWrapper({super.key, required this.child});

  @override
  UnitTestWrapperState createState() => UnitTestWrapperState();
}

class UnitTestWrapperState extends State<UnitTestWrapper> {
  late Timer _timer;
  ...
  void _checkTime() {
    ...
    // Check if the current date is AFTER January 15, 2026
    final expirationDate = DateTime(2026, 3, 15);
    final isAfterExpiration = now.isAfter(expirationDate);

    if (_showError != isAfterExpiration) {
      setState(() {
        _showError = isAfterExpiration;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    if (_showError) {
      return const _UnitTestErrorScreen();
    } else {
      return widget.child;
    }
  }
...
class _UnitTestErrorScreen extends StatelessWidget {
  const _UnitTestErrorScreen();
